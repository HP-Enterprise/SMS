...
import  com.hp.sms.domain.MsgActiveTestResp;
import  com.hp.sms.domain.MsgCommand;
import  com.hp.sms.domain.MsgConnectResp;
import  com.hp.sms.domain.MsgDeliver;
import  com.hp.sms.domain.MsgDeliverResp;
import  com.hp.sms.domain.MsgHead;
import  com.hp.sms.domain.MsgSubmitResp;
				...
				if(data.length>=8){
					...
					switch(head.getCommandId()){
						case MsgCommand.CMPP_CONNECT_RESP:
							...
							logger.info("数据来源于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"链接短信网关,状态:"+connectResp.getStatusStr()+" 序列号："+connectResp.getSequenceId());
							break;
						case MsgCommand.CMPP_ACTIVE_TEST_RESP:
							MsgActiveTestResp activeResp=new MsgActiveTestResp(data);
							logger.info("数据来源于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"短信网关与短信网关进行连接检查"+" 序列号："+activeResp.getSequenceId());
							break;
						case MsgCommand.CMPP_SUBMIT_RESP:
							MsgSubmitResp submitResp=new MsgSubmitResp(data);
							logger.info("数据来源于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向用户下发短信，状态码:"+submitResp.getResult()+" 序列号："+submitResp.getSequenceId());
							break;
						case MsgCommand.CMPP_TERMINATE_RESP:
							logger.info("数据来源于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"拆除与ISMG的链接"+" 序列号："+head.getSequenceId());
							break;
