...
import  com.hp.sms.domain.MsgConnect;
import  com.hp.sms.domain.MsgHead;
import  com.hp.sms.domain.MsgSubmit;
/**
 * 短信接口容器，单例获得链接对象
 *
 */
public class MsgContainer{
	...
	private static boolean sendShortMsg(String msg,String cusMsisdn){
		try {			
			...
			CmppSender sender=new CmppSender(getSocketDOS(),getSocketDIS(),dataList);
			sender.start();
			logger.info("数据来源于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向"+cusMsisdn+"下发短短信，序列号为:"+seq);
			return true;
		} catch (Exception e) {
			try {
				out.close();
	...
	private static boolean sendLongMsg(String msg,String cusMsisdn){
		try {
			...
			CmppSender sender=new CmppSender(getSocketDOS(),getSocketDIS(),dataList);
			sender.start();
			logger.info("数据来源于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向"+cusMsisdn+"下发长短信，序列号为:"+seqId);
			return true;
		} catch (Exception e) {
			try {
				out.close();
	...
	private static boolean sendShortWapPushMsg(String url,String desc,String cusMsisdn){
		try {
			...
			CmppSender sender=new CmppSender(getSocketDOS(),getSocketDIS(),dataList);
			sender.start();
			logger.info("数据来源于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向"+cusMsisdn+"下发web push短短信，序列号为:"+seq);
			return true;
		} catch (Exception e) {
			try {
				out.close();
	...
	private static boolean sendLongWapPushMsg(String url,String desc,String cusMsisdn){
		try {
			...
			CmppSender sender=new CmppSender(getSocketDOS(),getSocketDIS(),dataList);
			sender.start();
			logger.info("数据来源于"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"向"+cusMsisdn+"下发web pus长短信，序列号为:"+seqId);
			return true;
		} catch (Exception e) {
			try {
				out.close();
